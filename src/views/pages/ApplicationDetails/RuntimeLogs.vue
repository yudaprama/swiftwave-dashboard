  ...
  <PageBar>
    <template v-slot:title>
      <span class="flex flex-row items-center gap-2">
        {{ $t('applicationDetails.runtimeLogs') }}
        <StatusPulse type="success" :label="$t('applicationDetails.runtimeLogs')" />
      </span>
    </template>
    <template v-slot:subtitle>{{ $t('applicationDetails.runtimeLogsHint') }}</template>
    <template v-slot:buttons>
      <select
        class="focus:border-primary-500 focus:ring-primary-500 dark:bg-secondary-800 block rounded-md border-gray-300 shadow-xs sm:text-sm dark:border-gray-600 dark:text-gray-200"
