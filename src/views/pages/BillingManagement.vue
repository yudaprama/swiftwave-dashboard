...
const cancelSub = async () => {
  if (!confirm('Cancel your subscription? You can still use it until the end of the billing period.')) return
    ...
    <div class="mt-6 rounded-lg border bg-white p-6 shadow-sm">
      ...
      <div v-if="billingStore.subscription" class="mt-4">
        <div class="flex items-center gap-3">
          ...
          <span class="font-semibold">{{ billingStore.currentPlan?.name || 'Free' }}</span>
        ...
        <div class="mt-2 text-sm text-gray-600">
          <p>Period: {{ formatDate(billingStore.subscription.currentPeriodStart) }} - {{ formatDate(billingStore.subscription.currentPeriodEnd) }}</p>
          <p v-if="billingStore.subscription.cancelAtPeriodEnd" class="mt-1 text-yellow-700">
            ...
            Will be cancelled at end of period
          </p>
        </div>
        <button
      ...
      <p v-else class="mt-4 text-sm text-gray-500">No active subscription</p>
    </div>
